...
export type MonthExpenseHighlights = {
  ...
  /** Per category: how many expense rows, and the largest of them. */
  byCategory: {
    category: string;
    count: number;
    /** `recurring`: generated by a recurring rule, so a commitment rather than a one-off. */
...
// `byCategory` answers the same question for one category: the insight story
// needs to know whether a category's rise was one purchase or many, and the
// month's overall top three only covers categories that happen to hold them.
async function monthExpenseHighlights(
  ...
  const converted: (MonthExpenseHighlights['largest'][number] & { recurring: boolean })[] = [];
  for (const t of txs) {
    const amt = await txToAnchor(t);
    if (amt === null) continue;
    converted.push({
  ...
  // `converted` is sorted, so the first row seen for a category is its largest.
  const byCategory = new Map<string, MonthExpenseHighlights['byCategory'][number]>();
  for (const t of converted) {
    const seen = byCategory.get(t.category);
    if (seen) seen.count++;
    else {
      byCategory.set(t.category, {
        category: t.category,
        count: 1,
        largest: { description: t.description, amount: t.amount, date: t.date, recurring: t.recurring },
      });
    }
  }

  return {
    expenseCount: txs.length,
    largest: converted.slice(0, limit).map(({ recurring: _recurring, ...row }) => row),
    byCategory: [...byCategory.values()],
  };
}

// `-v2`: the result gained `byCategory`. A new key keeps an entry cached in the
// old shape from being read back without it.
const cachedMonthExpenseHighlights = cachedAggregation(
  ['month-expense-highlights-v2'],
  [CACHE_TAGS.transactions, CACHE_TAGS.categories, CACHE_TAGS.fx],
