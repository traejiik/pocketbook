...
export type StoryAction =
   ...
   * operating net the prompt already states. `category` is the lever — see
   * `gapLever` — or null when no day-to-day category exists, in which case the
   * advice is about day-to-day spending as a whole. `rose` says whether the lever
   * grew this month, so "back toward last month's" is only said when it did.
   * `purchase` / `renewal` name a concrete handle inside it when the data has one.
   */
  | {
      kind: 'close-gap'
...
/**
 * The two largest changes, whatever their size, for a month where nothing cleared
 * the floor. Stating them gives the note something real to say; the floor figure
 * itself is never shown, because July 2026's model read "no category changed by
 * more than 14 485 Ft" as "Housing increased by 14 485 Ft".
 */
export function smallMoves(s: InsightSnapshot): CategoryMove[] {
  return s.categories
    .map((c) => moveOf(s, c))
    .filter((m) => m.change !== 0)
...
/**
 * The category a gap can be closed in when nothing cleared the floor: the
 * day-to-day category (its largest expense not generated by a recurring rule)
 * that rose most, else the largest day-to-day category. Never a fixed cost —
 * July 2026 was told to "bring Housing back toward 150 000 Ft" when rent had not
 * moved at all.
 */
function gapLever(s: InsightSnapshot): CategoryMove | null {
  const dayToDay = s.categories.map((c) => moveOf(s, c)).filter((m) => !m.largest?.recurring)
  const rise = dayToDay.filter((m) => m.change > 0).sort((a, b) => b.change - a.change)[0]
  return rise ?? [...dayToDay].sort((a, b) => b.value - a.value)[0] ?? null
}
...
function actionFor(s: InsightSnapshot, headline: CategoryMove | null, gapAction: boolean): StoryAction {
  ...
  if (gapAction && (s.verdict === 'deficit' || s.verdict === 'tight')) {
    const lever = headline ?? gapLever(s)
    return {
      kind: 'close-gap',
      verdict: s.verdict,
      gap: Math.abs(s.kpis.operatingNet),
      category: lever?.name ?? null,
      value: lever?.value ?? null,
      prevValue: lever?.prevValue ?? null,
      rose: (lever?.change ?? 0) > 0,
      purchase:
        headline?.explainedBy === 'one-purchase' && headline.largest
          ...
          : null,
      renewal: lever ? renewalIn(s, lever.name) : null,
    }
  }
  if (!headline) {
