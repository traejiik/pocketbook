...
import { fmtAnchor, fmtDate, todayIso } from '@/lib/format';
import { nextOccurrence, parseDateOnly, formatDateOnly } from '@/lib/recurring-dates';
...
export interface SerializedRecurringRule {
  ...
  kind: 'INCOME' | 'EXPENSE' | 'SAVINGS';
  /** `YYYY-MM-DD` — the occurrence an early log would settle. */
  nextDue: string;
  amount: number;
  currency: 'HUF' | 'USD' | 'EUR' | 'GBP';
  categoryId: string;
}
...
export function TransactionForm({
  ...
  useEffect(() => {
    if (open && editingTx) {
      reset({
        ...
        logEarly: editingTx.coversDueDate != null,
      });
  ...
  // An edited transaction that already settled an occurrence keeps that rule; one
  // with a plain link (generated by the sync, or linked before early logging
  // existed) keeps its link read-only. Only a new settlement picks a rule.
  const settledOn = editingTx?.coversDueDate ?? null;
  const plainLink = editingTx && !settledOn && editingTx.recurringRuleId ? editingTx.recurringRuleName ?? 'a recurring rule' : null;
  const pickedRule = recurringRuleId ? eligibleRules.find(r => r.id === recurringRuleId) ?? null : null;
  const earlyNote = (() => {
    if (!logEarly) return null;
    if (settledOn) return { tone: 'muted' as const, text: `Settles the ${fmtDate(settledOn)} ${editingTx?.recurringRuleName ?? ''} payment. Turn off to hand it back to the rule.` };
    if (!pickedRule) return null;
    const due = parseDateOnly(pickedRule.nextDue);
    const after = formatDateOnly(nextOccurrence(pickedRule.cycle, due, due));
    if (dateVal > pickedRule.nextDue) {
      return { tone: 'warning' as const, text: `This payment was due ${fmtDate(pickedRule.nextDue)} — the scheduled entry may already exist.` };
    }
    return { tone: 'muted' as const, text: `Settles the ${fmtDate(pickedRule.nextDue)} payment · next due becomes ${fmtDate(after)}.` };
  })();

  function pickEarlyRule(id: string) {
    setValue('recurringRuleId', id);
    const rule = recurringRules.find(r => r.id === id);
    if (!rule) return;
    // Fill what is still empty from the rule; never overwrite what was typed.
    if (!amtStr) {
              ...
              {plainLink ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <Label htmlFor="tx-log-early" className="text-[13px] font-medium text-foreground">Log recurring early</Label>
                      <div className="text-[11px] text-muted-foreground mt-0.5">
                        Counts as this rule&apos;s next payment, so it won&apos;t be logged again on its due date
                      </div>
                    </div>
                    <Switch
                      id="tx-log-early"
                      checked={logEarly}
                      disabled={!settledOn && eligibleRules.length === 0}
                      onCheckedChange={(v) => {
                        setValue('logEarly', v);
                        if (!v && !settledOn) setValue('recurringRuleId', null);
                      }}
                    />
                  </div>
                  {logEarly && !settledOn && (
                    <Select value={recurringRuleId ?? ''} onValueChange={v => v && pickEarlyRule(v)}>
                      <SelectTrigger id="tx-recurring" aria-label="Recurring rule" className="h-9! w-full">
                        <SelectValue>{pickedRule ? `${pickedRule.name} · due ${fmtDate(pickedRule.nextDue, { short: true })}` : 'Pick a rule'}</SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        {eligibleRules.map(r => (
                          <SelectItem key={r.id} value={r.id}>
                            {r.name} · due {fmtDate(r.nextDue, { short: true })}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  {earlyNote && (
                    <p className={cn('text-[11.5px]', earlyNote.tone === 'warning' ? 'text-warning' : 'text-muted-foreground')}>
                      {earlyNote.text}
                    </p>
                  )}
                  {!settledOn && eligibleRules.length === 0 && (
                    <p className="text-[11.5px] text-muted-foreground">No active {type.toLowerCase()} rules.</p>
                  )}
                </div>
              )}
