...
export function NotificationsBell({ className }: { className?: string }) {
  ...
  const triggerClasses = cn(
    ...
    open ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-accent/70',
    className,
  );
  ...
  const list = (
    <div className="pb-1.5 max-h-[60vh] md:max-h-[320px] overflow-auto">
      {items.map((it) => {
        const Icon = it.icon;
        return (
          <div key={it.id} className="flex items-start gap-2.5 px-4 py-[9px] border-t border-border/40">
            <span
              className={cn(
                'w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-px',
                it.unread ? 'bg-primary/15 text-primary' : 'bg-secondary text-muted-foreground',
              )}
            >
            ...
            <span className="flex-1 min-w-0">
              ...
              <span className="block text-[10.5px] text-muted-foreground mt-0.5 tabular">{it.time}</span>
            </span>
          </div>
        );
      })}
  ...
  const empty = (
    <div className="px-4 pb-4 text-[12px] text-muted-foreground">Nothing yet — activity will collect here.</div>
  );

  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetTrigger aria-label="Notifications" className={triggerClasses}>
          {triggerInner}
        </SheetTrigger>
        <SheetContent
          side="bottom"
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger aria-label="Notifications" className={triggerClasses}>
        {triggerInner}
      </PopoverTrigger>
      <PopoverContent
        align="end"
