...
export function TransactionImportReview({ open, onOpenChange, filename, rows, categories, onImported }: Props) {
            ...
            {groups.new.map((r) => {
              ...
              return (
                <li key={r.line} className={cn('px-3 py-2.5 transition-opacity', !choice?.include && 'opacity-60')}>
                  ...
                  {(() => {
                    // Category hints end in "pick one"; once one is picked they are resolved.
